...
public class SwaggerConfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("ssafyVueJS").select()
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.happyhouse.controller")).paths(regex("/vue/.*")).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("SSAFY HappyHouse REST API").description(
