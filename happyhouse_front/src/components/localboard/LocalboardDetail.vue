            ...
            <div class="row justify-content-center">
              <div class="card-profile-actions py-4 mt-lg-0">
                ...
                <div class="float-right">
                  <b-button type="default" size="sm" @click="moveModifyBoard"
                    >update</b-button
                  >
                  <b-button size="sm" @click="deleteBoard">delete</b-button>
                </div>
              </div>
...
export default {
  ...
  computed: {
    ...
    message() {
      if (this.localboard.content)
        return this.localboard.content.split("\n").join("<br>");
      return "";
    },
  },
  created() {
    ...
    getBoard(
      ...
      (response) => {
        this.localboard = response.data;
        this.comment.boardno = this.localboard.boardno;
      },
      (error) => {
  ...
  methods: {
    ...
    moveModifyBoard() {
      this.$router.replace({
        ...
        params: { boardno: this.localboard.boardno },
      });
      //   this.$router.push({ path: `/notice/modify/${this.article.articleno}` });
    },
    deleteBoard() {
      if (confirm("삭제하시겠습니까?")) {
        deleteBoard(this.localboard.boardno, () => {
          this.$router.push({ name: "localboardList" });
        });
      }
    },
    ...
    userCheck() {
      return this.localboard.userid === this.userInfo.userid;
    },
  },
};
</script>
